  ...
  return new Promise((resolve, reject) => {
    ...
    RECIPE_URLS.forEach(async (url) => {
      ...
      try {
        ...
        const newRecipe = await fetch(url);

        //Is this how I'm supposed to fetch the urls??? When I try to open the site nothing shows up

        // A7. For each fetch response, retrieve the JSON from it using .json().
        //     NOTE: .json() is ALSO asynchronous, so you will need to use
        //     "await" again
        const newRecipeJSON = await newRecipe.json();

        // A8. Add the new recipe to the recipes array
        newRecipes.push(newRecipeJSON);
